...
def get-pending-tickets [] {
    db-query "SELECT jira_key, summary, description, status, manual FROM tickets WHERE status = 'pending' ORDER BY created_at ASC"
}

def get-stale-not-ready-tickets [] {
    let cutoff = ((date now) - $NOT_READY_COOLDOWN | format date "%Y-%m-%dT%H:%M:%S")
    db-query $"SELECT jira_key, summary, description, status, manual FROM tickets WHERE status = 'not_ready' AND updated_at < '($cutoff)' ORDER BY created_at ASC"
}
